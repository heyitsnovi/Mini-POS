 $('.product_status').val($('input[name="_productstatus"]').val());
 $('.product_category').val($('input[name="_productcategory"]').val());
 $('.product_supplier').val($("input[name='_supplierID']").val()!=='' ? $("input[name='_supplierID']").val() : 0);
...
$(document).on('click','.btn-upload-product-img',function(){
 ...
 $(document).on('click','.btn-updateproduct',function(){

 		$.ajax({
 			url: '/admin/product/save-update',
 			type: 'POST',
